...
class InterviewSystem:
    ...
    def __init__(self, openai_api_key: str, config: Optional[InterviewConfig] = None):
        ...
        if self.use_ollama:
            ...
            self.llm = ChatOpenAI(
                model="deepseek-r1",  # or "llama3", "mistral", "codellama", etc.
            ...
            self.embeddings = OllamaEmbeddings(
                model="granite-embedding:30m",  # or "all-minilm", "mxbai-embed-large"
        ...
        # Initialize specialized components (same for both)
    ...
    def process_candidate_answer(self, state: InterviewState, answer: str) -> InterviewState:
        ...
        # Ensure current_question is set (safeguard)
        if not state.get('current_question') and state.get('current_question_idx') is not None:
            if state.get('interview_plan') and state['current_question_idx'] < len(state['interview_plan']):
                state['current_question'] = state['interview_plan'][state['current_question_idx']]['question']
