...
from app.services.models import InterviewState
...
class InterviewWorkflowManager:
    ...
    def _retrieve_context(self, state: InterviewState) -> InterviewState:
        ...
        try:
            search_query = f"{state.get('current_question', '')} {state.get('candidate_response', '')}"
            state['rag_context'] = self.interview_system.rag_system.get_context(search_query)
            print("✅ Context retrieved successfully")
            
        except Exception as e:
            print(f"❌ Error in _retrieve_context: {e}")
            import traceback
    ...
    def _analyze_response(self, state: InterviewState) -> InterviewState:
        ...
        if not state.get('candidate_response'):
            print("⚠️ No candidate response to analyze")
            return state
        
        if not state.get('current_question'):
            print("⚠️ No current question for analysis")
            return state
        ...
        try:
            ...
            print(f"❌ Error in _analyze_response: {e}")
            import traceback
            print(f"Full traceback: {traceback.format_exc()}")
            # Use fallback analysis
            state['current_analysis'] = f"Analysis failed: {e}"
    ...
    def _take_notes(self, state: InterviewState) -> InterviewState:
        ...
        if not state.get('candidate_response'):
            print("⚠️ No candidate response found, skipping note taking")
            return state
        
        if not state.get('current_question'):
            print("⚠️ No current question found, skipping note taking")
            return state
        
        if not state.get('interview_plan') or state.get('current_question_idx') is None:
            print("⚠️ Interview plan or question index not found, using default category")
            question_category = 'general'
        else:
            try:
                question_category = state['interview_plan'][state['current_question_idx']].get('category', 'general')
            except (IndexError, KeyError, TypeError):
                print("⚠️ Could not get question category, using default")
                question_category = 'general'
        ...
        try:
            ...
            print(f"✅ Note taken successfully, moving to question {state['current_question_idx'] + 1}")
            
        except Exception as e:
            print(f"❌ Error in _take_notes: {e}")
            import traceback
            print(f"Full traceback: {traceback.format_exc()}")
