...
> Vendored from [bitbonsai/mcpvault](https://github.com/bitbonsai/mcpvault) (MIT). The server lives at `vendor/mcpvault/`; this skill drives it.
...
The MCP server (`obsidian`) is wired in `.mcp.json` and exposes the tools below. Reach for the vault when the user talks about notes/memory/knowledge; reach for `docs/` when producing a planning artifact.
